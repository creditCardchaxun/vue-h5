          ...
          <van-rate v-model="value"  :size="25" :change='valueChange(value,currentIndex)'/>
            <p v-show='toshows'> 
               <!-- <span @click="toaddTag(item,index)" :class='{tagons:tags}' v-for='(item,index) in iconList' :key='index'>
			 	       ...
			 	       <span v-for="(b,i) in item" :key='i' v-show='index+1==value' @click="toaddTag($event,b,index)" :class='{tagons:tags}'> 
                           {{b}}
			 	      </span>
			        </em>
            ...
            <p><textarea name="" v-model='user_desc' id="" cols="30" rows="3" :placeholder="$t('m.appraise2')"></textarea></p>
