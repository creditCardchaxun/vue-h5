...
<aheaders status='3' @toback='toreplace'></aheaders>
    ...
    <div class="order-detail01"> 
      <div class="order-left" style='position:relative;'>
          <h3 style='position:absolute;top:-5px;left:0px;'>{{$t('m.bookdetail1')}}</h3>
           ...
           <em style='width:4.2rem;color:#929292;font-size:0.3rem;margin-top: 0.8rem;line-height:0.6rem;display:block;margin-top:0.2rem;'>
    ...
    <div class="order-detail02">
        <!-- v-show='showStar' -->
        <div class="pingjia"  v-if="feedbackarr.user_score"><em>{{$t('m.bookdetail2')}}</em>      <van-rate v-model="value" readonly  /></div>
    ...
    <div class="yijiedan" v-show="showSale">
      <div class="yi-01"><img :src="sales.avater" alt="">
         <div class="yi_02"><h3>{{sales.realname}}</h3> <p>{{sales.sale_desc}}</p></div>
      </div>
     
       <div class="yi_03" @click='callPhone(sales.mobile)'><img src="../../assets/images/dianhua.png" alt=""></div>
    </div>

    <div class="order-detail03">
       <p><span>{{$t('m.feedback12')}}</span> <em>{{bookarr.book_number}}</em>  <em style='color:#929292;font-size:0.3rem;margin-left:0.2rem;' @click="copy" class='getcopy' :data-clipboard-text="bookarr.book_number">{{$t('m.feedback14')}}</em> </p>
       <p><span>{{$t('m.feedback13')}}</span> <em>{{bookarr.inputtime}}</em> </p>
    </div>
...
.order-detail .order-detail01 .order-left p{font-size:0.38rem;color:#060606;line-height: 0.6rem;margin-top:0.8rem;}
.order-detail .order-detail01 .order-right{width:3.69rem;height:2.65rem;}
.order-detail .order-detail01 .order-right img{width:100%;height:100%;}

.order-detail02{width:auto;height:auto;border-bottom:0.2rem solid #f5f5f5;display: flex;align-items: left;flex-direction: column;padding:0 0.6rem 0.78rem;}
.order-detail02 p{font-size:0.35rem;margin-top:0.73rem;color:#060606;display: flex;align-items:center;justify-content:flex-start;}
.order-detail02 span{color:#929292;width:2.3rem;display: block;}
...
.order-detail03{width:auto;height:2.96rem;display: flex;align-items:left;flex-direction: column;padding:0 0.6rem;justify-content: center;}
.order-detail03 p{font-size:0.35rem;line-height:0.96rem;color:#060606;display: flex;align-items:center;justify-content:flex-start;}
.order-detail03 span{color:#929292;width:2.3rem;}
.order-detail03 em{color: #060606;font-size: 0.36rem;}

.kefu{display: flex;align-items:center;justify-content:center;height:1.8rem;border-bottom:0.2rem solid #f5f5f5;border-top:0.2rem solid #f5f5f5;}
.kefu img{width:0.35rem;height:0.35rem;margin-right:0.3rem;}
.kefu span{font-size:0.35rem;color:#000000;}

.pingjia{width:auto;height:0.5rem;display: flex;align-items: center;justify-content:space-between;margin-top:0.66rem;}
.pingjia em{font-size:0.4rem;color:#060606;font-weight: bold;}
.pingjia .van-rate{width:auto;height:auto;}
</style>